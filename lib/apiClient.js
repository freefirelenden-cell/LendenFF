...
// get all accounts 
export async function getAccounts(id, page = 1, limit = 12) {
  try {
    const res = await fetch(`${BASE_URL}/api/accounts?userId=${id || ""}&page=${page}&limit=${limit}`, {
      cache: "no-store",
    });
...
export async function getAccountById(id, includePassword = false) {
  try {
    const res = await fetch(`${BASE_URL}/api/accounts/${id}?includePassword=${includePassword}`, {
      cache: "no-store",
    });
    const data = await res.json();
    if (!res.ok) throw new Error(data.error);
...
export async function updateUser(sellerId, data) {
  const res = await fetch(`/api/auth/user/${sellerId}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
...
export async function getAllSellers(role = "seller") {
  try {
    const res = await fetch(`/api/auth/user?role=${role}`, {
      cache: "no-store",
    });
