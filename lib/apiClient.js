...
const BASE_URL = process.env.NODE_ENV === 'production' 
  ? 'https://freefirelenden.vercel.app' 
  : 'http://localhost:3000';

export async function getNewest(limit) {
  try {
    console.log(BASE_URL)
    const res = await fetch(`${BASE_URL}app/api/accounts?sortBy=newest&limit=${limit}`, {
      cache: "no-store",
    });
    ...
    console.error("get accounts error error:", err.message);
    return null;
  }
}
...
//     return data;
//   } catch (err) {
//     console.error("getSellerAccounts error:", err.message);
//     return null;
//   }
// }
