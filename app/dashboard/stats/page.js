...
export default function StatsPage() {
  const { userCreatedAccounts, isLoadedUserCreatedAccounts, isLoaded } = useContext(myContext);
  const [stats, setStats] = useState(null);

  useEffect(() => {
    if (!isLoaded || !userCreatedAccounts) return;
  ...
  }, [isLoaded, userCreatedAccounts]);


  
  if (!isLoaded || !stats || !isLoadedUserCreatedAccounts) {
